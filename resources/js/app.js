...
(() => {
    ...
    // Safe class helpers (work for HTML & SVG)
    const getClass = (el) =>
    ...
    // Changed WeakMap → Map so we can iterate
    /** @type {Map<Element,[string,string]>} */
    ...
    const scan = (root) => {
        root.querySelectorAll('[class*="dark:"]').forEach((el) => {
            ...
            if (dark) setClass(el, isDark() ? dark : light);
        });
    };
    ...
    new MutationObserver((muts) => {
        ...
        muts.forEach((m) => {
            m.addedNodes.forEach((n) => {
                ...
                if (cls.includes(DARK_MARK)) snap(n);
    ...
    new MutationObserver(() => requestAnimationFrame(applyTheme)).observe(
        document.documentElement,
        { attributes: true, attributeFilter: [DARK_ATTR] },
    );
